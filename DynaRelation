...
function drawTables(vTableAll, vxStart, vyStart, vHeight, vColumnOffset) {
    ...
    var Lcount = 0;
    $.each(vTableAll, function (key, value) {
        if (value.Relational == '0') {
            vHeightTable = heightofTable(value, vHeight);
            fnSetTablePosition(value.datadef, vHeightTable + vyNRStart + voffsetNR, count);
            if (count == 0) {
                drawNewTableIndep(vxNRStart, vyNRStart, vHeight, value);
                vyNRStart += vHeightTable + voffsetNR;
                vWithTable = widthofTable(value) * vtblwidthFactor;

                if (vmaxwidthNR < vWithTable) {
                    vmaxwidthNR = vWithTable;
                    vmaxWidthAll += vmaxwidthNR;
                }
            }
            else {
                drawNewTableIndep(vxNRStart, vyNRStart, vHeight, value);
                ...
                vyNRStart += vHeightTable + voffsetNR;
                vWithTable = widthofTable(value) * vtblwidthFactor;
                //fnSetTablePosition(value.datadef, vHeightTable + vyNRStart);
                if (vmaxwidthNR < vWithTable) {
                    vmaxwidthNR = vWithTable;
                    vmaxWidthAll = vmaxwidthNR;
                }
            ...
            vHeightTable = heightofTable(value, vHeight);
            //            vmaxHeightCol += vHeightTable + vyStart + voffsetNR;
            vmaxHeightCol += vHeightTable + voffsetNR;
            //            vCol++;
            //            vRow = 0;
            fnSetTablePosition(value.datadef, vmaxHeightCol, Mcount);
            if (Mcount == 0) {
                vyLStart = vyStart;
                if (vmaxWidthAll != 0) {
                    vmaxWidthAll += vxStart + vColumnOffset + vCheckWidth;
                }
                else {
                    vmaxWidthAll = vxStart;
                }

                //vRow = 0;
                drawNewTableIndep(vmaxWidthAll, vyLStart, vHeight, value);
                vyLStart += vHeightTable + voffsetNR;
                vWithTable = widthofTable(value) * vtblwidthFactor;
                vCheckWidth = vWithTable;
                vmaxHeightCol = vHeightTable + vyStart;
            }
            else {
                drawNewTableIndep(vmaxWidthAll, vyLStart, vHeight, value);
                //vHeightTable = heightofTable(value, vHeight);
...
function drawNewTableIndep(xStart, yStart, vheight, vtblData) {
    var vtblwidthFactor = 9;
    var vwidth = widthofTable(vtblData) * vtblwidthFactor + 2;
    var vrow = vtblData.data.length;
    var voffset = 4
    var vXAxis = xStart;
    var vYAxis = yStart;
    var vYIncre = vheight;
    ...
    for (var i = 0; i < vtblData.data.length; i++) {
        if (vtblData.data[i][1] == '1') {
            ctx.fillStyle = 'rgb(255,221,0)';
        }
        else if (vtblData.data[i][1] == '0') {
            ctx.fillStyle = 'rgb(102,204,0)';
        }
        else if (vtblData.data[i][1] == '2') {
    ...
    ctx.fillText(vtblData.datadef, xStart - vlinewidth + voffset, yStart - vlinewidth - voffset);
    for (var i = 0; i < vtblData.data.length; i++) {
        ctx.font = '10pt Verdana';
        ctx.fillText(vtblData.data[i][0], xStart + voffset, (yStart - vlinewidth + (i + 1) * vYIncre - voffset));
...
function fnMaxWidhColPos() {
    for (var i = 0; i < vArrFromTo.length; i++) {
        ...
        //        if (vCountMaxWidth == 0) {
        //            vMaxWidhColPos[vCountMaxWidth] = new Array();
        //            vMaxWidhColPos[vCountMaxWidth].push(vContent[8]);
        //            vMaxWidhColPos[vCountMaxWidth].push(vContent[3]);
        //            vCountMaxWidth++;
        //        }
        //        if (vCountMaxWidth + 1 == vContent[8]) {
        //            if (vMaxWidhColPos[vCountMaxWidth - 1][1] < vContent[3]) {
        //                vMaxWidhColPos[vCountMaxWidth - 1][0] = vContent[3];
        //                vMaxWidhColPos[vCountMaxWidth] = vContent[8]
        //            }
        //        }
